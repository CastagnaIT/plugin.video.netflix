...
def _compile_remove_tvshow_tasks(videoid):
    results = g.SHARED_DB.get_all_episodes_ids_and_filepath_from_tvshow(videoid.value)
    return _create_remove_tv_tasks(results)


def _compile_remove_season_tasks(videoid):
    results = g.SHARED_DB.get_all_episodes_ids_and_filepath_from_season(
...
def _create_remove_tv_tasks(results):
    return [_create_remove_item_task(_episode_title_from_path(result[0]),
                                     result[0],
...
def export_item(item_task, library_home):
    ...
    common.debug('Destination folder={}'.format(destination_folder))
...
def _remove_videoid_from_db(videoid):
    ...
    # TODO: Remove unneded code
    #  Regarding the code, there's no need of those conditions. Only Episodes and Movies are deleted
    # elif videoid.mediatype == common.VideoId.SHOW:
